...
function Get-AcmeConnectorRegistry {
    $entries = @(
        ...
        New-AcmeConnectorRegistryEntry -ConnectorId 'kemp' -Label 'Kemp LoadMaster' -NoobDescription 'Issue a certificate and run the Kemp LoadMaster deployment hook from saved device profile and selected virtual services.' -Category 'network_appliance' -ScriptFileName 'cert2kemp.ps1' -FirstRunAcmeSupported $true -PostIssuanceDeploySupported $true -DefaultStorePlugin 'pfxfile,certificatestore' -DefaultScriptParameters "-PfxPath '{CacheFile}' -CachePassword '{CachePassword}'" -RequiresPfx $true -RequiresManualTargetConfig $true
        New-AcmeConnectorRegistryEntry -ConnectorId 'clavister' -Label 'Clavister NetWall / cOS Core' -NoobDescription 'Issue a certificate and run the Clavister SSH/SCP deployment hook from a saved device profile.' -Category 'network_appliance' -ScriptFileName 'cert2clavister.ps1' -FirstRunAcmeSupported $true -PostIssuanceDeploySupported $true -DefaultStorePlugin 'pfxfile,certificatestore' -DefaultScriptParameters "-PfxPath '{CacheFile}' -CachePassword '{CachePassword}'" -RequiresPfx $true -RequiresManualTargetConfig $true
        ...
        New-AcmeConnectorRegistryEntry -ConnectorId 'paloalto' -Label 'Palo Alto firewall' -NoobDescription 'Issue a certificate and run the Palo Alto XML/REST deployment hook from a saved device profile.' -Category 'network_appliance' -ScriptFileName 'cert2paloalto.ps1' -FirstRunAcmeSupported $true -PostIssuanceDeploySupported $true -DefaultStorePlugin 'pfxfile,certificatestore' -DefaultScriptParameters "-PfxPath '{CacheFile}' -CachePassword '{CachePassword}' -CertCommonName '{CertCommonName}'" -RequiresPfx $true -RequiresManualTargetConfig $true
