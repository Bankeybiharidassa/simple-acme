...
$DeviceSchemas = @{
    ...
    paloalto = @{ ConnectorType='paloalto'; Label='Palo Alto firewall'; Category='network_appliance'; SetupMode='guided'; Fields=@(
        ...
        @{ Name='cert_path'; Label='Certificate PEM path'; Type='string'; Required=$false; Placeholder='C:\certs\pa.itsecured.nl.crt.pem'; HelpText='Optional PEM certificate path for manual deploy-paloalto.ps1 runs. Renewal hooks can inject this at runtime.' },
        @{ Name='key_path'; Label='Private key PEM path'; Type='string'; Required=$false; Placeholder='C:\certs\pa.itsecured.nl.key.pem'; HelpText='Optional PEM private key path for manual deploy-paloalto.ps1 runs. Contents are never displayed in logs.' },
        @{ Name='chain_path'; Label='Chain PEM path'; Type='string'; Required=$false; Placeholder='C:\certs\chain.pem'; HelpText='Optional PEM chain path when operators keep the leaf certificate and chain separately.' },
        @{ Name='key_passphrase'; Label='Private key passphrase'; Type='secret'; Required=$false; Placeholder=''; HelpText='Optional passphrase for encrypted PEM private keys. PAN-OS accepts short passphrases; keep it 31 characters or fewer.' },
        ...
        @{ Name='binding_target'; Label='Binding target'; Type='string'; Required=$false; Placeholder=''; HelpText='Leave empty for management. For GlobalProtect use portal:<name> or gateway:<name>; for WAF/decryption use the rule or target name.' },
        ...
        @{ Name='bind_target_names'; Label='Selected Palo Alto targets'; Type='string'; Required=$false; Placeholder=''; HelpText='Human-readable summary of selected Palo Alto deployment targets. The management binding uses PAN-OS deviceconfig/system and does not need a named target.' }
    )}
