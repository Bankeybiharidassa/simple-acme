...
if ($null -eq $cert) {
    ...
    Import-PfxCertificate -FilePath $PfxPath -Password $pfxPassword -CertStoreLocation 'Cert:\LocalMachine\My' | Out-Null
    $cert = Get-ChildItem Cert:\LocalMachine\My -ErrorAction SilentlyContinue | Where-Object { $_.Thumbprint -eq $normalized } | Select-Object -First 1
    if ($null -eq $cert) { Write-Error 'Certificate not found after import.'; exit 1 }
}
if (-not $cert.HasPrivateKey) { Write-Error "Certificate $normalized has no private key."; exit 1 }
