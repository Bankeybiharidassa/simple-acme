...
$apply = {
    ...
    $record = [ordered]@{ connector='firewall'; host=[string]$endpoint.host; thumbprint=[string]$cert.Thumbprint; updatedAt=(Get-Date).ToUniversalTime().ToString('o') }
    $path = Join-Path $stateDir ("fw-{0}.json" -f ([string]$endpoint.host).ToLowerInvariant())
    ($record | ConvertTo-Json -Depth 4 -Compress) | Set-Content -LiteralPath $path -Encoding UTF8
    Write-ConnectorLog -Component 'cert2fw' -Action 'apply' -Target ([string]$endpoint.host) -Result 'success' -Details @{ stateFile = $path; thumbprint = $cert.Thumbprint } -EmitConsole
}
