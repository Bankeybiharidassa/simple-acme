...
$DeviceSchemas = @{
    ...
    sophos = @{ ConnectorType='sophos'; Label='Sophos firewall'; Category='network_appliance'; Fields=@(
        ...
        @{ Name='certificate_name'; Label='Name in Sophos'; Type='string'; Required=$true; Placeholder='wildcard_itsecured_nl'; HelpText='Name to give this certificate inside Sophos. Letters, numbers, dash and underscore are safest.' },
        @{ Name='pfx_path'; Label='Certificate PFX file'; Type='string'; Required=$false; Placeholder='C:\certs\_.itsecured.nl.pfx'; HelpText='Path to the issued .pfx file. The setup fills this from C:\certs when possible.' },
        @{ Name='pfx_password'; Label='PFX password'; Type='secret'; Required=$false; Placeholder=''; HelpText='Password for the .pfx file. Use the password chosen during certificate setup.' },
        @{ Name='pfx_password_secret_name'; Label='PFX password secret'; Type='string'; Required=$false; Placeholder='SOPHOS_PFX_PASSWORD'; HelpText='Advanced: name of a saved secret/env var for the PFX password. Leave empty when using PFX password above.' },
        @{ Name='pfx_password_secure_file'; Label='PFX password file'; Type='string'; Required=$false; Placeholder='C:\secrets\sophos-pfx-password.txt'; HelpText='Advanced: path to a DPAPI-encrypted PFX password file. Leave empty when using PFX password above.' },
        @{ Name='cert_path'; Label='PEM cert file'; Type='string'; Required=$false; Placeholder='C:\certs\fullchain.pem'; HelpText='Alternative to PFX: path to fullchain.pem. Leave empty when using a PFX file.' },
        @{ Name='key_path'; Label='PEM key file'; Type='string'; Required=$false; Placeholder='C:\certs\privkey.pem'; HelpText='Alternative to PFX: path to privkey.pem. Leave empty when using a PFX file.' },
        @{ Name='chain_path'; Label='PEM chain file'; Type='string'; Required=$false; Placeholder=''; HelpText='Optional extra chain file for PEM mode. Usually empty.' },
        ...
        @{ Name='enable_ssh_export_recovery'; Label='Use SSH recovery'; Type='choice'; Required=$true; Choices=@('false','true'); Default='false'; Placeholder='false'; HelpText='Advanced diagnostics fallback. Leave false unless API export recovery is needed.' },
        @{ Name='ssh_username'; Label='SSH username'; Type='string'; Required=$false; Default='admin'; Placeholder='admin'; HelpText='Only used when SSH recovery is true.' },
        @{ Name='ssh_port'; Label='SSH port'; Type='string'; Required=$false; Default='22'; Placeholder='22'; HelpText='Only used when SSH recovery is true. Default is 22.' },
        @{ Name='ssh_password_secret_name'; Label='SSH password secret'; Type='string'; Required=$false; Placeholder='SOPHOS_SSH_PASSWORD'; HelpText='Only used when SSH recovery is true and no SSH key is used.' },
        @{ Name='ssh_private_key_path'; Label='SSH private key'; Type='string'; Required=$false; Placeholder='C:\keys\sophos.ppk'; HelpText='Only used when SSH recovery is true and key login is used.' },
        @{ Name='ssh_host_key_fingerprint'; Label='SSH host fingerprint'; Type='string'; Required=$false; Placeholder='SHA256:...'; HelpText='Only used when SSH recovery is true. Protects against connecting to the wrong firewall.' },
        @{ Name='export_recovery_path'; Label='SSH recovery output'; Type='string'; Required=$false; Placeholder='C:\temp\sophos-export.tar'; HelpText='Only used when SSH recovery is true. Temporary file path for recovered export data.' }
    )}
