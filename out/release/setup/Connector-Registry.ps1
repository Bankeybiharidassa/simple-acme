...
function Get-AcmeConnectorRegistry {
    $entries = @(
        ...
        New-AcmeConnectorRegistryEntry -ConnectorId 'sophos' -Label 'Sophos firewall' -NoobDescription 'Available after certificate issuance / post-deployment only. Use the Sophos deployment target workflow.' -Category 'network_appliance' -ScriptFileName 'deploy-sophos.ps1' -FirstRunAcmeSupported $false -PostIssuanceDeploySupported $true -DefaultStorePlugin 'pfxfile,certificatestore' -DefaultScriptParameters "-PfxPath '{CacheFile}' -CertificateName '{CertThumbprint}'" -RequiresPfx $true -RequiresManualTargetConfig $true
