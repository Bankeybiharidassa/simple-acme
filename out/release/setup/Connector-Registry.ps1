...
function Get-AcmeConnectorRegistry {
    $entries = @(
        ...
        New-AcmeConnectorRegistryEntry -ConnectorId 'sophos' -Label 'Sophos firewall' -NoobDescription 'Available after certificate issuance / post-deployment only. Configure firewall deployment separately.' -Category 'network_appliance' -ScriptFileName 'deploy-sophos.ps1' -FirstRunAcmeSupported $false -PostIssuanceDeploySupported $true -RequiresPfx $true -RequiresManualTargetConfig $true
